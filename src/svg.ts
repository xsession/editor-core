...
function renderStroke(pathData: string, layer: WirePaintLayer, className: string, extra = ''): string {
...
function selected(selection: SelectionState | undefined, kind: string, id: string): boolean {
  return Boolean(selection?.items.some((item) => item.kind === kind && item.id === id));
}

function portTextAttributes(side: string): { anchor: 'start' | 'middle' | 'end'; dx: number; dy: number } {
  switch (side) {
    case 'east':
...
function renderComponent(
  ...
  for (const port of component.ports.filter((candidate) => candidate.visible).sort((a, b) => a.order - b.order)) {
    ...
    if (showPorts) rows.push(`<circle data-port-id="${escapeXml(port.id)}" cx="${formatNumber(portGeometry.center.x)}" cy="${formatNumber(portGeometry.center.y)}" r="${formatNumber(style.pinDotRadius)}" fill="${escapeXml(style.pinDotFill)}" stroke="${escapeXml(style.pinDotStroke)}" stroke-width="1.5" vector-effect="non-scaling-stroke"/>`);
...
function renderWire(
  ...
  if (!wire.route || wire.route.points.length < 2 || wire.hidden) return '';
  ...
  if (paint.selection) strokes.push(renderStroke(routePath, paint.selection, 'editor-core-wire-selection'));
  if (paint.warning) strokes.push(renderStroke(routePath, paint.warning, 'editor-core-wire-warning'));
  if (paint.outline) strokes.push(renderStroke(routePath, paint.outline, 'editor-core-wire-outline'));
  for (const layer of paint.engineeringLayers) strokes.push(renderStroke(routePath, layer, 'editor-core-wire-stroke'));
...
function renderCrossingBridges(document: EditorDocument, theme: EditorTheme, background: string): string {
...
function resolvedLabelStyle(label: LabelNode, theme: EditorTheme): TextStyle {
...
function renderLabel(label: LabelNode, placement: LabelPlacement, theme: EditorTheme, selection?: SelectionState): string {
...
function renderRouteHandles(document: EditorDocument, selection: SelectionState | undefined, theme: EditorTheme): string {
  const result: string[] = [];
...
function documentBounds(
  ...
  const rects: Rect[] = [];
  for (const geometry of Object.values(geometries)) rects.push(geometry.worldBounds);
  for (const wire of Object.values(document.wires)) if (wire.route?.points.length) rects.push(routeBounds(wire.route.points, wire.style.width + 10));
  for (const placement of placements) rects.push(placement.bounds);
  if (rects.length === 0) return { x: -500, y: -300, width: 1000, height: 600 };
...
export interface SvgInteractionOverlay {
  ...
  marquee?: { bounds: Rect; mode: 'window' | 'crossing' };
  connection?: SvgConnectionPreview;
  statusText?: string;
}

function renderSnapGuides(guides: readonly SnapGuide[], theme: EditorTheme): string {
...
function previewRoutePoints(start: Point, end: Point): Point[] {
  if (Math.abs(start.x - end.x) < 1e-6 || Math.abs(start.y - end.y) < 1e-6) return [start, end];
...
function renderInteractionOverlay(
  document: EditorDocument,
  geometries: Record<string, ComponentGeometry>,
  overlay: SvgInteractionOverlay | undefined,
  theme: EditorTheme,
): string {
  if (!overlay) return '';
  const result: string[] = [];
  if (overlay.snapGuides?.length) result.push(renderSnapGuides(overlay.snapGuides, theme));
  if (overlay.marquee) {
...
export function renderEditorSvg(document: EditorDocument, context: SvgRenderContext): string {
  ...
  const wires = document.wireOrder
    ...
    .filter((wire): wire is WireEdge => Boolean(wire))
  ...
  const components = document.componentOrder
    ...
    .filter((geometry): geometry is ComponentGeometry => Boolean(geometry))
...
export function renderSvgFragment(document: EditorDocument, context: SvgRenderContext): string {
  const svg = renderEditorSvg(document, context);
  return svg.slice(svg.indexOf('>') + 1, svg.lastIndexOf('</svg>'));
}

export function contentBoundsFromSvgContext(document: EditorDocument, context: SvgRenderContext): Rect {
