...
import type { EditorDocument, LabelNode, WireColorPattern, WireEdge, WireEndpoint } from './types.js';
...
function wire(
  ...
  routePattern: WireEdge['routing']['pattern'] = 'orthogonal',
  requestedRadius = 10,
): WireEdge {
  return {
    id,
...
function label(id: string, text: string, ownerId: string, ownerKind: 'component' | 'wire', priority = 10): LabelNode {
  return {
    id,
    text,
...
export function createSampleEngine(): HarnessEditorEngine {
