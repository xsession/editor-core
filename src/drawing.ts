...
function renderTable(element: DrawingElement, theme: EditorTheme): string {
  ...
  const lines: string[] = [`<rect x="0" y="0" width="${formatNumber(element.width)}" height="${formatNumber(element.height)}" fill="${escapeXml(fill)}" fill-opacity="0.94" stroke="${escapeXml(stroke)}" vector-effect="non-scaling-stroke"/>`,
  ...
  columns.forEach((column, index) => {
    ...
    if (index) lines.push(`<line x1="${formatNumber(x)}" y1="${titleHeight}" x2="${formatNumber(x)}" y2="${formatNumber(element.height)}" stroke="${escapeXml(stroke)}" stroke-opacity="0.55" vector-effect="non-scaling-stroke"/>`);
...
export function renderDrawingElements(elements: readonly DrawingElement[] | undefined, theme: EditorTheme): string {
