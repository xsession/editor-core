...
export type RouteConstraintKind =
  | 'waypoint'
  | 'locked-waypoint'
  | 'corridor-horizontal'
  | 'corridor-vertical'
...
export type WireColorPattern =
  ...
  | { kind: 'stripe'; base: string; stripe: string; stripeWidth: number; repeat: number }
  | { kind: 'tracer'; base: string; tracer: string; repeat: number; tracerLength: number }
  | { kind: 'dual'; primary: string; secondary: string; ratio: number }
  | { kind: 'shield'; sheath: string; core?: string }
  | { kind: 'custom'; layers: WirePaintLayer[] };
...
export type ComponentLabelCandidate =
  | 'north'
  | 'north-east'
  | 'east'
  | 'south-east'
...
export interface EditorEventMap {
  documentChanged: { document: EditorDocument; reason: string; impact?: MutationImpact };
  selectionChanged: { selection: SelectionState };
  validationChanged: { issues: ValidationIssue[] };
  feedback: EditorFeedback;
  historyChanged: { canUndo: boolean; canRedo: boolean; undoLabel?: string; redoLabel?: string };
  interactionChanged: { state: string };
}
